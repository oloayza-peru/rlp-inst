...
# Importación de componentes de AgGrid para los filtros tipo Excel
...
st.caption(
    "Plataforma Gerencial de Monitoreo de KPIs de Inspección, Seguridad y"
    " Operaciones"
)
...
def load_default_data():
  try:
    ...
    df_sst = pd.DataFrame(
        columns=["Tipo", "Descripción", "Mes", "% Cumplimiento"]
    )

  try:
    df_inst_plan = pd.read_excel(
        "KPI PLAN INSTRUMENTACION.xlsx", sheet_name="Plan"
    )
  except Exception:
    df_inst_plan = pd.DataFrame(
        columns=[
            "PLAN",
            "TIPO",
            "MES",
  ...
  try:
    df_inst_vaar = pd.read_excel(
        "KPI PLAN INSTRUMENTACION.xlsx", sheet_name="Válvulas VAAR- Sensores"
    )
  except Exception:
    df_inst_vaar = pd.DataFrame(
        columns=[
            "PLAN",
            "TIPO",
            "MES",
...
def to_excel_download(df, sheet_name="Datos"):
  output = io.BytesIO()
  cols_to_drop = [
      c for c in ["Year_Temp", "Month_Temp", "Month_Num_Temp"] if c in df.columns
  ]
...
def filter_df(df_in, years, months):
  ...
  if (
      years
      and "Year_Temp" in df_out.columns
      and df_out["Year_Temp"].notnull().any()
  ):
    df_out = df_out[df_out["Year_Temp"].isin(years)]
  if (
      months
      and "Month_Temp" in df_out.columns
      and df_out["Month_Temp"].notnull().any()
  ):
...
def plot_monthly_trend(
    df_input,
    date_col="MES",
    title="Tendencia Mensual: Programados vs. Inspeccionados (100%)",
):
...
with tab_resumen:
  ...
  df_inst_full = pd.concat(
      [st.session_state["df_inst_plan"], st.session_state["df_inst_vaar"]],
      ignore_index=True,
  )

  df_sst_proc, y_sst, m_sst = extract_year_month(
      st.session_state["df_sst"], "Mes"
  )
  df_inst_proc, y_inst, m_inst = extract_year_month(df_inst_full, "MES")
  df_acc_proc, y_acc, m_acc = extract_year_month(
      st.session_state["df_acc"], "FECHA"
  )
  df_ops_proc, y_ops, m_ops = extract_year_month(
      st.session_state["df_ops_gen"], "FECHA"
  )
  ...
  with col_y:
    sel_years_res = st.multiselect(
        "📅 Filtrar por Año (Resumen General):",
        options=all_years,
        default=all_years,
        key="filter_res_year",
    )
  with col_m:
    sel_months_res = st.multiselect(
        "🗓️ Filtrar por Mes (Resumen General):",
        options=all_months,
        default=all_months,
  ...
  avg_sst = (
      (df_sst_r["% Cumplimiento"].mean() * 100)
      if not df_sst_r.empty and "% Cumplimiento" in df_sst_r.columns
      else 0
  )
  col1.metric("Cumplimiento SST", f"{avg_sst:.1f}%")

  avg_inst = (
      (df_inst_r["AVANCE DE CAMPO"].mean() * 100)
      if not df_inst_r.empty and "AVANCE DE CAMPO" in df_inst_r.columns
      else 0
  )
...
with tab_sst:
  ...
  uploaded_sst = st.file_uploader(
      "Reemplazar/Actualizar Matriz SST (Excel)", type=["xlsx"], key="u_sst"
  )
  if uploaded_sst:
    st.session_state["df_sst"] = pd.read_excel(uploaded_sst)
    st.success("Matriz SST actualizada correctamente.")

  df_sst_proc, years_sst, months_sst = extract_year_month(
      st.session_state["df_sst"], "Mes"
  )

  col_f1, col_f2 = st.columns(2)
  with col_f1:
    sel_years_sst = st.multiselect(
        "📅 Filtrar por Año (SST):",
        options=years_sst,
        default=years_sst,
        key="filter_sst_year",
  ...
  if not df_sst_filtered.empty:
    ...
    with c2:
      avg_val = (
          df_sst_filtered["% Cumplimiento"].mean() * 100
          if "% Cumplimiento" in df_sst_filtered.columns
          else 0
      )
      fig_gauge = go.Figure(
          go.Indicator(
  ...
  cols_to_show = [
      c
      for c in df_sst_filtered.columns
      if c not in ["Year_Temp", "Month_Temp", "Month_Num_Temp"]
  ]
  edited_sst = st.data_editor(
      df_sst_filtered[cols_to_show], num_rows="dynamic", key="editor_sst"
  )
...
with tab_inst:
  ...
  uploaded_inst = st.file_uploader(
      "Reemplazar/Actualizar Matriz Instrumentación (Excel)",
      type=["xlsx"],
      key="u_inst",
  )
  if uploaded_inst:
    ...
    if "Plan" in xls.sheet_names:
      st.session_state["df_inst_plan"] = pd.read_excel(
          uploaded_inst, sheet_name="Plan"
      )
    if "Válvulas VAAR- Sensores" in xls.sheet_names:
      st.session_state["df_inst_vaar"] = pd.read_excel(
          uploaded_inst, sheet_name="Válvulas VAAR- Sensores"
      )
  ...
  with subtab_plan:
    ...
    with c1:
      s_yp = st.multiselect(
          "📅 Año:", options=years_p, default=years_p, key="f_p_y"
      )
    with c2:
      s_mp = st.multiselect(
          "🗓️ Mes:", options=months_p, default=months_p, key="f_p_m"
      )
    with c3:
      units_plan = (
          sorted([
    ...
    pct_p_avance = (
        (total_p_ejec / total_p_prog * 100) if total_p_prog > 0 else 0.0
    )
    ...
    fig_p_monthly = plot_monthly_trend(
        df_p_filt,
        title=(
            "Indicador Mensual: Programados vs. Inspeccionados al 100% (Plan"
            " General)"
        ),
    )
    if fig_p_monthly:
  ...
  with subtab_vaar:
    ...
    with c1:
      s_yv = st.multiselect(
          "📅 Año:", options=years_v, default=years_v, key="f_v_y"
      )
    with c2:
      s_mv = st.multiselect(
          "🗓️ Mes:", options=months_v, default=months_v, key="f_v_m"
      )
    with c3:
      units_vaar = (
          sorted([
    ...
    if s_uv and "UNIDAD" in df_v_filt.columns:
      df_v_filt = df_v_filt[
          df_v_filt["UNIDAD"].astype(str).isin([str(u) for u in s_uv])
      ]

    total_v_prog = len(df_v_filt)
    total_v_ejec = (
        int((df_v_filt["AVANCE DE CAMPO"] == 1).sum())
        if "AVANCE DE CAMPO" in df_v_filt.columns
        else 0
    )
    pct_v_avance = (
        (total_v_ejec / total_v_prog * 100) if total_v_prog > 0 else 0.0
    )
    ...
    fig_v_monthly = plot_monthly_trend(
        df_v_filt,
        title=(
            "Indicador Mensual: Programados vs. Inspeccionados al 100%"
            " (Válvulas VAAR)"
        ),
    )
    if fig_v_monthly:
  ...
  with subtab_sensores:
    ...
    with c1:
      s_ys = st.multiselect(
          "📅 Año:", options=years_s, default=years_s, key="f_s_y"
      )
    with c2:
      s_ms = st.multiselect(
          "🗓️ Mes:", options=months_s, default=months_s, key="f_s_m"
      )
    with c3:
      units_sens = (
          sorted([
    ...
    if s_us and "UNIDAD" in df_s_filt.columns:
      df_s_filt = df_s_filt[
          df_s_filt["UNIDAD"].astype(str).isin([str(u) for u in s_us])
      ]

    total_s_prog = len(df_s_filt)
    total_s_ejec = (
        int((df_s_filt["AVANCE DE CAMPO"] == 1).sum())
        if "AVANCE DE CAMPO" in df_s_filt.columns
        else 0
    )
    pct_s_avance = (
        (total_s_ejec / total_s_prog * 100) if total_s_prog > 0 else 0.0
    )
    ...
    fig_s_monthly = plot_monthly_trend(
        df_s_filt,
        title=(
            "Indicador Mensual: Programados vs. Inspeccionados al 100%"
            " (Sensores de Vibración)"
  ...
  st.download_button(
      label=(
          "📥 Descargar Matriz Consolidada de Instrumentación Actualizada"
      ),
...
with tab_acc:
  ...
  uploaded_acc = st.file_uploader(
      "Subir Archivo de Accidentes (Excel)", type=["xlsx"], key="u_acc"
  )
  if uploaded_acc:
    st.session_state["df_acc"] = pd.read_excel(
        uploaded_acc, sheet_name="SEG_ACCIDENTES"
    )
    st.success("Registro de accidentes actualizado.")

  df_acc_proc, years_acc, months_acc = extract_year_month(
      st.session_state["df_acc"], "FECHA"
  )

  col_f1, col_f2 = st.columns(2)
  with col_f1:
    sel_years_acc = st.multiselect(
        "📅 Filtrar por Año:",
        options=years_acc,
        default=years_acc,
        key="filter_acc_year",
  ...
  if not df_acc_filtered.empty:
    ...
    with c2:
      if (
          "TIPO" in df_acc_filtered.columns
          and "UBICACIÓN" in df_acc_filtered.columns
      ):
        fig_acc_tipo = px.bar(
            df_acc_filtered,
            x="TIPO",
  ...
  cols_to_show_acc = [
      c
      for c in df_acc_filtered.columns
      if c not in ["Year_Temp", "Month_Temp", "Month_Num_Temp"]
  ]
  edited_acc = st.data_editor(
      df_acc_filtered[cols_to_show_acc], num_rows="dynamic", key="editor_acc"
  )
...
with tab_ops:
  ...
  uploaded_ops = st.file_uploader(
      "Subir Matriz OPS (Excel)", type=["xlsx"], key="u_ops"
  )
  if uploaded_ops:
    st.session_state["df_ops_gen"] = pd.read_excel(
        uploaded_ops, sheet_name="OPS_GENERADAS"
    )
    st.session_state["df_ops_cuota"] = pd.read_excel(
        uploaded_ops, sheet_name="OPS_CUOTA"
    )
    st.success("Control de OPS actualizado.")

  df_ops_proc, years_ops, months_ops = extract_year_month(
      st.session_state["df_ops_gen"], "FECHA"
  )

  col_f1, col_f2 = st.columns(2)
  with col_f1:
    sel_years_ops = st.multiselect(
        "📅 Filtrar por Año:",
        options=years_ops,
        default=years_ops,
        key="filter_ops_year",
  ...
  if not df_ops_filtered.empty:
    ...
    with c1:
      if (
          "ÁREA" in df_ops_filtered.columns
          and "GRAVEDAD" in df_ops_filtered.columns
      ):
        fig_ops_area = px.bar(
            df_ops_filtered,
            x="ÁREA",
  ...
  cols_to_show_ops = [
      c
      for c in df_ops_filtered.columns
      if c not in ["Year_Temp", "Month_Temp", "Month_Num_Temp"]
  ]
  edited_ops = st.data_editor(
      df_ops_filtered[cols_to_show_ops], num_rows="dynamic", key="editor_ops"
  )
